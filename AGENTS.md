...
- **Hebrew vs English prompts are separate.** When the effective reply language is Hebrew, system-prompt instructional copy (block titles, craft rules, grounding, rewrite nudges, automations, recovery) must be Hebrew — no English scaffolding mixed in. When English, use English copy. Put Hebrew strings in `ai/hebrew-reply-style.ts` (or a sibling helper), not inline English with a Hebrew reply body. Add/extend tests that assert Hebrew prompts do not contain English rule phrases and vice versa.
