...
export async function buildRelationshipMap(
  ...
  const scored = [...pairs.values()].map((pair) => {
    const totalReplies =
      pair.signals.reply_count_a_to_b + pair.signals.reply_count_b_to_a
    return {
      ...pair,
      interaction_score: Math.min(1, totalReplies / 50),
      conflict_score: Math.min(1, pair.signals.disagreement_count / 10),
      solidarity_score: Math.min(
        1,
        (pair.signals.agreement_count + pair.signals.defense_count) / 20,
      ),
    }
  })
...
async function generateNarrativesBatch(
  ...
  const pairDescriptions = pairs.map((p, idx) => {
    const s = p.signals
