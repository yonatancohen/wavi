      ...
      >
        ...
        <div class="mb-3 flex flex-wrap gap-2">
          ...
          <span class="rounded-full bg-secondary/10 px-2.5 py-1 text-[10px] font-semibold text-secondary">
            {{ member.profile_data.emoji_usage }} emoji
          </span>
          <span
        ...
        <div
          v-if="member.profile_data.sensitivity_flags.length > 0"
          class="flex flex-wrap gap-1.5 border-t border-outline-variant/50 pt-3"
        >
          <span
            v-for="flag in member.profile_data.sensitivity_flags"
            :key="flag"
            class="rounded-md bg-error/[0.06] px-2 py-0.5 text-[10px] text-on-surface-variant/60"
          >
            {{ flag }}
          </span>
        </div>
