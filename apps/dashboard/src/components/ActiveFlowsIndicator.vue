<template>
  <div
    v-if="total > 0"
    class="rounded-xl border border-secondary/25 bg-secondary/[0.06]"
  ...
  >
    <div class="flex items-center gap-2" :class="compact ? '' : 'mb-2'">
      ...
      <p
        class="font-mono font-semibold uppercase tracking-widest text-secondary"
    ...
    <ul v-if="!compact" class="space-y-1.5">
      <li
        v-for="flow in flows"
        :key="flow.id"
        class="rounded-lg border border-on-surface/[0.05] bg-surface-container-high/40 px-2.5 py-2"
      >
        ...
        <p v-if="flow.message_preview" class="mt-0.5 truncate text-[11px] italic text-on-surface-variant/80">
          "{{ flow.message_preview }}"
        </p>
      </li>
    </ul>
  </div>
