<template>
  <div class="flex flex-col rounded-xl border border-outline-variant bg-surface-container p-5">
    ...
    <div v-if="!connected && !connecting" class="rounded-lg border border-outline-variant/60 bg-surface-container-high/40 p-4">
      ...
      <RouterLink
        to="/connect"
        class="mt-3 inline-flex items-center gap-1 text-[12px] font-semibold text-primary no-underline hover:underline"
      >
    ...
    <dl v-else class="flex flex-1 flex-col gap-2.5">
      <div
        v-for="row in rows"
        :key="row.key"
        class="flex items-start justify-between gap-3 rounded-lg border border-on-surface/[0.05] bg-surface-container-high/35 px-3 py-2.5"
      >
        <dt class="text-[11px] font-medium text-on-surface-variant">{{ row.label }}</dt>
        <dd
          class="text-end font-mono text-[11px] font-semibold"
          :class="row.toneClass"
        >
          {{ row.value }}
        </dd>
    ...
    <p
      v-if="healthTier === 'degraded'"
      class="mt-3 text-[11px] leading-relaxed text-on-surface-variant"
    >
