<template>
  <div ref="wrapperRef" class="relative inline-flex" @mouseenter="onEnter" @mouseleave="show = false">
    <!-- Badge -->
    <div class="flex items-center gap-1.5 rounded-full border cursor-default" :class="[compact ? 'px-2 py-1' : 'px-2.5 py-1', badgeClass]">
      <div class="rounded-full" :class="[compact ? 'h-1.5 w-1.5' : 'h-2 w-2', dotClass]" />
      <span class="font-mono font-semibold uppercase tracking-widest" :class="compact ? 'text-[9px]' : 'text-[10px]'">
        {{ label }}
      </span>
    </div>
        ...
        <div class="p-3">
          ...
          <template v-if="healthTier === 'degraded' && health">
            <div class="mt-2.5 space-y-1.5 border-t border-outline-variant/60 pt-2.5">
              <div v-for="row in degradedDetails" :key="row.label" class="flex items-center justify-between gap-3">
          ...
          <div v-if="healthTier === 'healthy' && phoneNumber" class="mt-2.5 border-t border-outline-variant/60 pt-2.5">
...
const { healthTier, health, phoneNumber } = storeToRefs(agentStore);
...
const tooltipTitle = computed(() => {
  switch (healthTier.value) {
    case 'healthy':
      return t('status.tooltip.healthyTitle');
    case 'degraded':
      return t('status.tooltip.degradedTitle');
    case 'connecting':
      return t('status.tooltip.connectingTitle');
    default:
      return t('status.tooltip.offlineTitle');
  }
});
...
const tooltipBody = computed(() => {
  switch (healthTier.value) {
    case 'healthy':
      return t('status.tooltip.healthyBody');
    case 'degraded':
      return t('status.tooltip.degradedBody');
    case 'connecting':
      return t('status.tooltip.connectingBody');
    default:
      return t('status.tooltip.offlineBody');
  }
});
