  ...
  <section class="flex flex-col overflow-hidden rounded-xl border border-outline-variant bg-surface-container">
    ...
    <div v-else ref="scrollEl" class="flex max-h-[min(560px,60vh)] flex-col overflow-y-auto px-4 py-3" @scroll="onScroll">
      ...
      <div class="flex flex-col gap-3">
        <article v-for="msg in messages" :key="msg.id" class="flex flex-col gap-1" :class="msg.is_agent_reply ? 'items-end' : 'items-start'">
          <div class="flex max-w-[85%] flex-col gap-1" :class="msg.is_agent_reply ? 'items-end' : 'items-start'">
            <div class="flex flex-wrap items-center gap-x-2 gap-y-0.5 px-0.5" :class="msg.is_agent_reply ? 'flex-row-reverse' : ''">
              <span class="text-[11px] font-semibold" :class="msg.is_agent_reply ? 'text-primary' : 'text-on-surface'">
            ...
            <div
              ...
              :class="msg.is_agent_reply ? 'rounded-br-md bg-primary/12 text-on-surface' : 'rounded-bl-md border border-outline-variant/60 bg-surface-variant/30 text-on-surface'"
            >
              {{ msg.body }}
            </div>
          </div>
